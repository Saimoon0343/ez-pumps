...
class AddEditPump extends Component {
    ...
    onAddEditPump = () => {
        const {id, selectedState, selectedJobType, selectedPumpType, line_length, volume, pump_name, description, availability} = this.state;

        if(selectedState && selectedJobType && selectedPumpType && line_length && volume && pump_name && description){
            let availabilityStatus = false;
            Object.keys(availability).map((item) => {
                if(availability[item].am_slot === true || availability[item].pm_slot === true){
    ...
    render() {
        ...
        const {loading, failed, pump_form_info, delete_alert, delete_loading} = this.props;
                    ...
                    failed
                        ...
                        <View style={styles.main}>
                            <ScrollView showsVerticalScrollIndicator={false}>
                                ...
                                {Object.keys(availability).map((item) => (
                                    <Availability Day={item} availability={availability[item]}
...
const mapDispatchToProps = dispatch => {
    return {
        createUpdatePump: (pumpId, state, jobType, pumpType, lineLength, volume, pumpName, description, availability) => dispatch(createUpdatePump(pumpId, state, jobType, pumpType, lineLength, volume, pumpName, description, availability)),
        showOrHideDeleteAlert: (delete_alert) => dispatch({type: SHOW_OR_HIDE_DELETE_PUMP_ALERT, delete_alert: delete_alert}),
